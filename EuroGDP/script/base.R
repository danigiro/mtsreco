...
freq <- 4
test_date_h1 <- data$date[data$date>="2010 Q1"]
...
for(i in test_date_h1){
  ...
  pb <- progress_bar$new(format = paste0("Replication n. ", which(test_date_h1 == i), 
...
save(base_array, res_array, test_array, scale, model, file = "./data/base.RData")
